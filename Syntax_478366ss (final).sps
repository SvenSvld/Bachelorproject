...
WEIGHT by anweight.
* Maak een filter aan welke Ierland en mensen die niet gestemd hebben uit de analyse haalt.
USE ALL.
...
EXECUTE.
USE ALL.
COMPUTE filter_$=((NMIS(hinctnta,eduyrs,agea,Threat,dummy_rightwing,R_gndr,R_blgetmg,
    dummy_grootstedelijk,dummy_stedelijk,dummy_landelijk,dummy_werkloos,dummy_sociaaldemocratisch,
    dummy_samenwonend,MIPEX,perc_migranten,rlgdgr)=0) AND (cntry~='IE') AND (vote=1)).
VARIABLE LABELS filter_$ "(NMIS(hinctnta,eduyrs,agea,Threat,dummy_rightwing,R_gndr,R_blgetmg,"+
    "dummy_grootstedelijk,dummy_stedelijk,dummy_landelijk,dummy_werkloos,dummy_sociaaldemocratisch,"+
    "dummy_samenwonend,MIPEX,perc_migranten,rlgdgr)=0) AND (cntry~='IE') AND (vote=1... (FILTER)".
VALUE LABELS filter_$ 0 'Not Selected' 1 'Selected'.
FORMATS filter_$ (f1.0).
FILTER BY filter_$.
EXECUTE.
...
CTABLES
  /VLABELS VARIABLES=dummy_rightwing Threat dummy_sociaaldemocratisch perc_migranten MIPEX agea 
    R_gndr R_blgetmg eduyrs hinctnta rlgdgr dummy_werkloos dummy_samenwonend dummy_grootstedelijk 
    dummy_stedelijk dummy_landelijk cntry 
    DISPLAY=LABEL
  /TABLE dummy_rightwing [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] 
    + Threat [VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + 
    dummy_sociaaldemocratisch [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] 
    + perc_migranten [VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + MIPEX 
    [VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + agea [VALIDN F40.0, 
    MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + R_gndr [S][VALIDN F40.0, MINIMUM F40.2, 
    MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + R_blgetmg [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM 
    F40.2, MEAN F40.2, STDDEV F40.2] + eduyrs [VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, 
    STDDEV F40.2] + hinctnta [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] 
    + rlgdgr [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + dummy_werkloos 
    [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + dummy_samenwonend 
    [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + dummy_grootstedelijk 
    [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + dummy_stedelijk 
    [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] + dummy_landelijk 
    [S][VALIDN F40.0, MINIMUM F40.2, MAXIMUM F40.2, MEAN F40.2, STDDEV F40.2] BY cntry
...
EXECUTE.
* (2) Voer logistische regressie uit.
LOGISTIC REGRESSION VARIABLES dummy_rightwing
  /METHOD=ENTER Threat perc_migranten Cross_Threat.Percmigranten agea R_gndr R_blgetmg eduyrs 
    dummy_deciel2 dummy_deciel3 dummy_deciel4 dummy_deciel5 dummy_deciel6 dummy_deciel7 
    dummy_deciel8 dummy_deciel9 dummy_deciel10 rlgdgr dummy_werkloos dummy_stedelijk 
    dummy_grootstedelijk dummy_samenwonend 
  /CRITERIA=PIN(0.05) POUT(0.10) ITERATE(20) CUT(0.5).
...
**** Hypothese 3a: Inwoners uit landen met een goed integratiebeleid hebben minder kans op steun voor een rechts-populistisch 
    partij dan inwoners uit een land met een slecht integratiebeleid.
LOGISTIC REGRESSION VARIABLES dummy_rightwing
  /METHOD=ENTER MIPEX agea R_gndr R_blgetmg eduyrs dummy_deciel2 
    dummy_deciel3 dummy_deciel4 dummy_deciel5 dummy_deciel6 dummy_deciel7 
    dummy_deciel8 dummy_deciel9 dummy_deciel10 rlgdgr dummy_werkloos 
    dummy_stedelijk dummy_grootstedelijk dummy_samenwonend
  /CRITERIA=PIN(.05) POUT(.10) ITERATE(20) CUT(.5).
...
**** Hypothese 2b/3b: De relatie tussen sociaaldemocratische welvaartsstaatregimes en MIPEX op steun voor
    rechts-populistische partijen kan verklaard worden via waargenomen bedreiging ten aanzien van migranten
    
* (2b.1) Sociaaldemocratisch op steun rechts-populisme (c, totale effect).
* (2b.2) Sociaaldemocratisch en Threat op steun rechts-populisme (c', directe effect; pad b).
LOGISTIC REGRESSION VARIABLES dummy_rightwing
  /METHOD=ENTER dummy_sociaaldemocratisch Threat agea R_gndr R_blgetmg eduyrs dummy_deciel2 
    dummy_deciel3 dummy_deciel4 dummy_deciel5 dummy_deciel6 dummy_deciel7 
    dummy_deciel8 dummy_deciel9 dummy_deciel10 rlgdgr dummy_werkloos 
    dummy_stedelijk dummy_grootstedelijk dummy_samenwonend
  /CRITERIA=PIN(.05) POUT(.10) ITERATE(20) CUT(.5).
* (2b.3) Sociaaldemocratisch op Threat (pad a).
REGRESSION
  /MISSING LISTWISE
...
* (3b.1) MIPEX op steun voor rechts-populisme (c, totale effect).
* (3b.2) MIPEX en Threat op steun voor rechts-populisme (c', directe effect; pad b).
LOGISTIC REGRESSION VARIABLES dummy_rightwing
  /METHOD=ENTER MIPEX Threat agea R_gndr R_blgetmg eduyrs dummy_deciel2 
    dummy_deciel3 dummy_deciel4 dummy_deciel5 dummy_deciel6 dummy_deciel7 
    dummy_deciel8 dummy_deciel9 dummy_deciel10 rlgdgr dummy_werkloos 
    dummy_stedelijk dummy_grootstedelijk dummy_samenwonend
  /CRITERIA=PIN(.05) POUT(.10) ITERATE(20) CUT(.5).
* (3b.3) MIPEX op Threat (pad a).
REGRESSION
  /MISSING LISTWISE
  /STATISTICS COEFF OUTS R ANOVA
  /CRITERIA=PIN(.05) POUT(.10)
